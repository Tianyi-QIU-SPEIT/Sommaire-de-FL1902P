...
On note $\mathcal{P}(E)$ l'ensemble qui contient tous les sous-ensembles de $E$.
...
Attention: j'écris $f^{-1}(B)$ pour $f$ et $B$, mais $f$ n'est pas forcément bijective. 
...
En maths, on voyage à partir de quelques définitions et on trouve/déduire les propriétés, théorèmes, etc. Mais pour que ces assertions soient justifiées, il est indispensable de démontrer logiquement, avec ce qu'on sait.

Dans ce chapitre on va décrire comment rédiger une démonstration rigoureusement et correctement, avec des méthodes plus spécifiques qui se sont avérées efficaces lors de démonstrations.
